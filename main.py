...
import unicodedata
import base64
from diccionarios import map_fuerza
...
# --- TABLA ÚNICA: POSITIVOS POR GRUPO TÁCTICO ---
    ...
    df_m['Tipo'] = df_m['Tipo'].str.strip() # Limpiamos espacios extra
    
    # 2. Aplicamos el mapa PERO NO BORRAMOS nada
    df_m['GRUPO_TACTICO'] = df_m['Tipo'].map(map_fuerza).fillna('NO CLASIFICADOS')
    
    # 3. Crear matriz
    t_tactico = df_m.groupby(['Tipo', 'GRUPO_TACTICO']).size().unstack(fill_value=0)
    
    # 4. Asegurar columnas incluyendo la nueva
    cols_finales = ['CAPTURAS', 'EMERGENCIAS', 'RECUPERACIONES', 'SEGURIDAD VIAL', 'NO CLASIFICADOS']
    for col in cols_finales:
        if col not in t_tactico.columns: t_tactico[col] = 0
    
    # 5. Calcular totales
    t_tactico['TOTAL'] = t_tactico.sum(axis=1)
    
    # 6. Ordenar (aquí verás si el total llega a 4258)
    t_tactico_ordenada = t_tactico[cols_finales + ['TOTAL']].sort_values(by='TOTAL', ascending=False)
